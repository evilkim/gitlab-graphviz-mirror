...
static void svg_print_id_class(GVJ_t * job, char* id, char* idx, char* kind, void* obj)
{
    ...
    static const struct gv_xml_esc_options options = {
      .nbsp = true, .dash = true
    };

    gvputs(job, "<g id=\"");
    char *xml = gv_xml_esc_or_exit(id, options);
    gvputs(job, xml);
    free(xml);
    if (idx) {
	xml = gv_xml_esc_or_exit(idx, options);
	gvprintf (job, "_%s", xml);
	free(xml);
    }
    gvprintf(job, "\" class=\"%s", kind);
    if ((str = agget(obj, "class")) && *str) {
	gvputs(job, " ");
	xml = gv_xml_esc_or_exit(str, options);
	gvputs(job, xml);
	free(xml);
    }
    gvputs(job, "\"");
...
static void svg_comment(GVJ_t * job, char *str)
{
    ...
    gvputs(job, "<!-- ");
    char *xml = gv_xml_esc_or_exit(str, options);
    gvputs(job, xml);
    free(xml);
    gvputs(job, " -->\n");
}
...
static void svg_begin_job(GVJ_t * job)
{
    ...
    char *xml = gv_xml_esc_or_exit(job->common->info[0], options);
    gvputs(job, xml);
    free(xml);
    gvputs(job, " version ");
    xml = gv_xml_esc_or_exit(job->common->info[1], options);
    gvputs(job, xml);
    free(xml);
    gvputs(job, " (");
    xml = gv_xml_esc_or_exit(job->common->info[2], options);
    gvputs(job, xml);
    free(xml);
    gvputs(job, ")\n");
    gvputs(job, " -->\n");
...
static void svg_begin_graph(GVJ_t * job)
{
	...
	gvputs(job, " Title: ");
	char *xml = gv_xml_esc_or_exit(agnameof(obj->u.g), options);
	gvputs(job, xml);
	free(xml);
    }
...
static void svg_begin_node(GVJ_t * job)
{
    ...
    gvputs(job, ">\n");
    gvputs(job, "<title>");
    char *xml = gv_xml_esc_or_exit(agnameof(obj->u.n), options);
    gvputs(job, xml);
    free(xml);
    gvputs(job, "</title>\n");
}
...
static void svg_begin_edge(GVJ_t * job)
{
    ...
    char *xml = gv_xml_esc_or_exit(ename, options);
    gvputs(job, xml);
    free(xml);
    free(ename);
    gvputs(job, "</title>\n");
		 ...
		 char *id)
{
	...
	static const struct gv_xml_esc_options options = { 0 };
	char *xml = gv_xml_esc_or_exit(href, options);
	gvputs(job, xml);
	free(xml);
	gvputs(job, "\"");
    }
	...
	static const struct gv_xml_esc_options options = {
	  .raw = true, .nbsp = true, .dash = true
	};
	char *xml = gv_xml_esc_or_exit(tooltip, options);
	gvputs(job, xml);
	free(xml);
	gvputs(job, "\"");
    }
	...
	char *xml = gv_xml_esc_or_exit(target, options);
	gvputs(job, xml);
	free(xml);
	gvputs(job, "\"");
    }
...
static void svg_textspan(GVJ_t * job, pointf p, textspan_t * span)
{
	...
	char *xml = gv_xml_esc_or_exit(obj->id, options);
	gvprintf(job, "<textPath xlink:href=\"#%s_p\" startOffset=\"50%%\">", xml);
	free(xml);
    ...
    char *xml = gv_xml_esc_or_exit(span->str, options);
    gvputs(job, xml);
    free(xml);
    if (obj->labeledgealigned)
